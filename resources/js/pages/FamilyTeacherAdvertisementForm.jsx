...
const advertisementType = 'family_to_teacher';
...
export default function FamilyTeacherAdvertisementForm({
        ...
        async function load() {
            try {
                ...
                if (advertisementId) {
                    ...
                    if (advertisement.type !== advertisementType) {
                        throw new Error();
                    }
                    ...
                    setForm({
                        child_ids: (advertisement.children ?? []).map(
                            (child) => String(child.id)
                        ),
                        subject_ids: (advertisement.subjects ?? []).map(
                            (subject) => String(subject.id)
                        ),
                        format: advertisement.format ?? '',
                        participant_age_from: String(
                            advertisement.participant_age_from
                        ),
                        participant_age_to: String(
                            advertisement.participant_age_to
                        ),
                        city_id: String(advertisement.city_id),
                        district_id: String(
                            advertisement.district_id ?? ''
                        ),
                        metro_station_id: String(
                            advertisement.metro_station_id ?? ''
                        ),
    ...
    const districts = useMemo(
        () =>
            (locations.districts ?? []).filter(
                (item) => String(item.city_id) === form.city_id
            ),
        [locations.districts, form.city_id]
    );

    const stations = useMemo(
        () =>
            (locations.metro_stations ?? []).filter(
                (item) => String(item.city_id) === form.city_id
            ),
        [locations.metro_stations, form.city_id]
    );

    const closed = status === 'closed';

    function error(name) {
        return Array.isArray(errors[name])
            ? errors[name][0]
            : errors[name];
    }

    function inputClass(name) {
        return `block w-full rounded-xl border bg-white px-3.5 py-3 text-sm text-gray-900 outline-none transition disabled:cursor-not-allowed disabled:bg-gray-50 ${
            error(name)
    ...
    function toggleChild(id) {
        ...
        setForm((current) => ({
            ...current,
            child_ids: current.child_ids.includes(value)
                ? current.child_ids.filter((item) => item !== value)
                : [...current.child_ids, value],
        }));
    }
    ...
    async function send(url, method, body = null) {
        ...
        if (!response.ok) {
            const requestError = new Error(
                data.message ?? 'Не удалось сохранить объявление.'
            );
        ...
        try {
            const data = await send(
                advertisementId
                    ...
                    : '/api/advertisements',
                advertisementId ? 'PUT' : 'POST',
                payload()
            );

            if (!advertisementId) {
                window.location.href =
                    `/advertisements/${data.advertisement.id}/edit`;
    ...
    async function action(name) {
        ...
        try {
            const data = await send(
                `/api/advertisements/${advertisementId}/${name}`,
                'POST'
            );
        ...
        <div className="min-h-screen bg-[#f7f7f8] px-4 py-12 text-gray-900">
            <main className="mx-auto w-full max-w-2xl">
                ...
                <p className="mt-2 text-sm leading-6 text-gray-600">
                    Расскажите, какого педагога вы ищете и для кого нужны
                    занятия.
                </p>
                ...
                {message && (
                    <p
                        className={`mt-6 rounded-lg px-4 py-3 text-sm ${
                            ...
                            message.includes('опублик') ||
                            message.includes('закрыто')
                    ...
                    >
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                                Выберите детей, которым нужен педагог.
                            </p>
                            ...
                            <div className="mt-3 space-y-2">
                                {children.map((child) => (
                                    <label
                                        key={child.id}
                                        className="flex cursor-pointer items-center gap-3 rounded-xl border border-gray-200 px-4 py-3 text-sm text-gray-800 transition hover:bg-gray-50"
                                    >
                                        <input
                                            type="checkbox"
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            {[
                                ['participant_age_from', 'Возраст от'],
                                ['participant_age_to', 'Возраст до'],
                            ].map(([name, label]) => (
                                <div key={name}>
                                    <label
                                        htmlFor={name}
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <Select
                                label="Станция метро (необязательно)"
                                name="metro_station_id"
                                value={form.metro_station_id}
                                onChange={change}
                                options={stations}
                                error={error('metro_station_id')}
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                placeholder="Например: ищем преподавателя математики для регулярных занятий с ребёнком..."
